...
def Exam(event=0):
    ...
    if flag2:
        if flag0:
            ...
            flag0 = 1
            for i in r:
                t.insert('end', ws0.cell(i, 2).value)
                t.insert('end', '\n')
    else:
        for i in l:
            t.insert('end', ws0.cell(i, 2).value)
...
def show():
    var.set(ws0.cell(num - 1, 2).value)
    t1.delete('1.0', 'end')
    t1.insert('end', 'List：')
...
def Next(event=0):
    ...
    numlasat = num
    if v2.get() == 1:
        nnum = ws0.cell(num - 1, 0).value
        while True:
            nnum += 1
            for i in range(1, ws0.nrows):
                if ws0.cell(i - 1, 0).value == nnum:
                    num = i
            if v.get() == 2 or ws0.cell(num - 1, 5).value == v.get():
                break
    elif v2.get() == 2:
        nnum = ws0.cell(num - 1, 1).value
        while True:
            nnum += 1
            for i in range(1, ws0.nrows):
                if ws0.cell(i - 1, 1).value == nnum:
                    num = i
            if v.get() == 2 or ws0.cell(num - 1, 5).value == v.get():
                break
    else:
